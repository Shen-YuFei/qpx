...
| `pg_positions` | Not populated; this path does not obtain peptide-to-protein sequence coordinates. | Known peptide-evidence `start` / `end` coordinates for members of the resolved protein group, converted from zero-based inclusive to one-based inclusive. Null when no usable positions remain. A FASTA is not needed for coordinates already recorded in consensusXML. |
